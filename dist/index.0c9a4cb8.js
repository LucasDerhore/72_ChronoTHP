...
const PageDetail = (argument1)=>{
    ...
    const preparePage = ()=>{
        let cleanedArgument = argument1.replace(/\s+/g, "-");
        const showPlatforms = (platformsArray)=>{
            let array = [];
            for (let platform of platformsArray)array.push(platform.platform.name);
            return array.join(", ");
        };
        const showPublishers = (publishersArray)=>{
            let array = [];
            for (let publisher of publishersArray)array.push(publisher.name);
            return array.join(", ");
        };
        const showDevelopers = (developersArray)=>{
            let array = [];
            for (let developer of developersArray)array.push(developer.name);
            return array.join(", ");
        };
        ...
        const showGenres = (genresArray)=>{
            let array = [];
            for (let genre of genresArray)array.push(genre.name);
            return array.join(", ");
        };
        ...
        const fetchGame = (url1, argument)=>{
            let finalURL1 = url1 + argument + `?&key=${key}`;
            ...
            fetch(`${finalURL1}`).then((response)=>response.json()
            ).then((response)=>{
